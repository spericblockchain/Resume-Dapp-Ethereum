...
export class Web3Service {
  ...
  public async web3login() {
    RESHOSTERJSON = require('../../../../build/contracts/ResHoster.json')
    return new Promise(async (resolve, reject) => {
      ...
      if (window.ethereum || window.web3) {
        ...
        if (typeof RESHOSTERJSON.networks[Net] === 'undefined') {
        ...
        this.AccountSubscription = this.RefreshedAccount.subscribe(async () => {
          ...
          if (
            ...
            const ResHosterInstance = new window.web3.eth.Contract(
              RESHOSTERJSON.abi,
              RESHOSTERJSON.networks[Network].address
            )
            this.Web3Details$.next({
              account: Account,
              network: Network,
              reshoster: ResHosterInstance.methods
            })
          }
